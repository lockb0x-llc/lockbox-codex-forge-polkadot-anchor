...
import { anchorPolkadot } from "./polkadot-utils.js";
...
export async function handleLargeFileUpload(metadata, chunks) {
  try {
    ...
    } else if (metadata.anchorType === "polkadot") {
      try {
        const polkadotAccount = metadata.createdBy?.address || "5GrwvaEF5zXb26Fz9rcQpDWS57CtERHpNehXCPcNoHGKutQY";
        anchor = await anchorPolkadot(
          { id: uuidv4(), storage: { integrity_proof: integrity } },
          polkadotAccount,
        );
      } catch (err) {
        console.error("[large-file-handler] Polkadot anchor error:", err);
        return { ok: false, error: "Polkadot anchor error", details: err };
      }
    } else {
      try {
