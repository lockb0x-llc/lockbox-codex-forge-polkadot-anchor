...
import { anchorPolkadot } from "./protocol.js";
import { getPolkadotProfile, setPolkadotProfile } from "./polkadot-utils.js";
...
describe("polkadot-utils.js", () => {
  ...
  test("setPolkadotProfile rejects when profile missing address", async () => {
    ...
    await expect(setPolkadotProfile(invalidProfile)).rejects.toThrow(
      "Profile must include an address",
    );
  });

  test("setPolkadotProfile rejects when profile is null", async () => {
    await expect(setPolkadotProfile(null)).rejects.toThrow(
      "Profile must include an address",
    );
  });
