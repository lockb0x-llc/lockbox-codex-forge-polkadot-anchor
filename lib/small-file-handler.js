...
export async function handleSmallFileUpload(payload) {
  try {
    ...
    } else if (payload.anchorType === "polkadot") {
      try {
        const polkadotAccount = payload.createdBy?.address || "5GrwvaEF5zXb26Fz9rcQpDWS57CtERHpNehXCPcNoHGKutQY";
        anchor = await anchorPolkadot(
          { id: uuidv4(), storage: { integrity_proof: integrity } },
          polkadotAccount,
        );
      } catch (err) {
        console.error("[small-file-handler] Polkadot anchor error:", err);
